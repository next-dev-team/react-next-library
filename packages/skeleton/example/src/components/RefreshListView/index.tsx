...
export function RefreshListView<T>(props: RefreshListViewProps<T>) {
  ...
  const shouldStartHeaderRefreshing = () => {
    if (
      refreshState === RefreshStateEnum.HeaderRefreshing ||
      refreshState === RefreshStateEnum.FooterRefreshing
    ) {
      return false;
    }
  ...
  /**
   * Generate a component at the bottom of the page to display the prompt text or operation when you
